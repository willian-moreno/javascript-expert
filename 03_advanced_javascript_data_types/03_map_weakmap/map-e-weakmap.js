...
assert.deepStrictEqual(myMap.get('Willian'), { text: 'two' });
assert.deepStrictEqual(myMap.get(true)(), 'hello');

// Em Objects a chave só pode ser string ou symbol (number é coergido a string)
const onlyRefenceWorks = { id: 1 };
myMap.set(onlyRefenceWorks, { name: 'Willian' });

assert.deepStrictEqual(myMap.get({ id: 1 }), undefined);
assert.deepStrictEqual(myMap.get(onlyRefenceWorks), { name: 'WillianMoreno' });
...
// O jeito certo em Object é ({ name: 'Willian' }).hasOwnProperty('name')
